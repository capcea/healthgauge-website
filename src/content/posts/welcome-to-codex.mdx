export const meta = {
  title: 'Welcome to Codex: Why calculators and context matter',
  slug: 'welcome-to-codex',
  excerpt: 'A quick tour of the Codex platform and how we design calculators that empower everyday health decisions.',
  category: 'Product',
  date: '2024-11-18',
  author: 'Codex Editorial Team',
  tags: ['product', 'health-tech'],
  readingTime: '4 min'
};

import { AdSlot } from '../../components/AdSlot';

# Welcome to Codex

Codex began with a simple observation: most people know *what* they want to improve—sleep, energy, body composition—but feel lost when converting goals into numbers.

Our calculators follow three principles:

1. **Transparent methods.** Each tool cites its equation so you can double-check the logic.
2. **Actionable insights.** We accompany outputs with coaching cues to help you apply the numbers.
3. **Inclusive language.** Every page ships in English and Romanian, with more languages planned.

We also curate a newsroom with analyses on nutrition policy, longevity research, and wearable tech. Expect weekly digests that balance rigor with readability.

<AdSlot slotId="1234567890" />

Whether you are coaching clients or personalizing your own routine, Codex keeps data, context, and trustworthy sources in one tab.
